...
export class KeyDetector {
  static getKeyByContent(text: string) {
    ...
    for (const reg of regs) {
      (text.match(reg) || [])
        .forEach((rawKey) => {
          const key = Config.enableKeyPathParsing ? detectedKey(rawKey.replace(reg, '$1'), text) : rawKey
          keys.add(key)
        },
        )
    }
  ...
  static getKeyRange(document: TextDocument, position: Position, dotEnding?: boolean) {
    ...
    for (const regex of regs) {
      ...
      if (range) {
        ...
        const key = Config.enableKeyPathParsing ? detectedKey(rawKey, document.getText()) : rawKey
  ...
  static getKeys(document: TextDocument | string, regs?: RegExp[], dotEnding?: boolean, scopes?: ScopeRange[]): KeyInDocument[] {
    ...
    return Config.enableKeyPathParsing ? detectedKeys(keys, typeof document === 'string' ? document : document.getText()) : keys
  }
  ...
  static getUsages(document: TextDocument, loader?: Loader): KeyUsages | undefined {
    ...
    return {
      type,
      keys: Config.enableKeyPathParsing ? detectedKeys(keys, document.getText()) : keys,
      locale,
      namespace,
    }
  }
